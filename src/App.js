...
class App extends Component {
    ...
    async componentDidMount() {
        const fetchedData = await fetchData(); // Fetch global data initially
        this.setState({ data: fetchedData });
        console.log('Initial Data:', fetchedData);
    }

    handleCountryChange = async (country) => {
        const fetchedData = await fetchData(country); // Fetch data for the selected country
        this.setState({ data: fetchedData, country: country });
        console.log('Fetched Data for Country:', fetchedData);
    }
